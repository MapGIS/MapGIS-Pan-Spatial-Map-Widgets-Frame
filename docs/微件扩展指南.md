...
5.在 src/widgets/kuozhanweijianmulu下新建widgets.js、setting.js、export.js

widgets.js : 导出当前目录下所有微件

settings.js : 导出当前目录下所有的配置UI

export.js : 按需导出文件

![](./images/widgets4.png)

6.在src/widgets文件中执行操作：①导入按需导出的文件  ②导入微件并注册

![](./images/widgets5.png)

在src/settings.js文件中执行操作: ①导入按需导出的文件  ②导入配置UI并注册
...
方式一：源码yarn link方式，特别注意此时必须将mapgis-pan-spatial-map-widgets-frame包当中package.json的软件包入口点改成src/index.js,即采用module的引入方式
